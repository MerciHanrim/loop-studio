...
| **LTA-INV-4** | **Locked / mobile:** read-only. The current classification (§LTA4.1) renders as its preset name (`unsupported` shows only §LTA4.4's message, nothing else), plus the one §LTA6.2 group line — no per-option reason text, since there are no options; no radios, no `select`, no keyboard target. Mirrors `docs/register-expression-authoring.md` §RXA5's mobile rule verbatim. |
...
Locked / mobile (LTA-INV-4) has no options to put a per-radio reason beside —
there are no radios. It renders only: the currently stored preset's name
(`phase0`/`afterPull`, skipped entirely for `unsupported` — §LTA4.4's message
already states the whole situation), plus the **one** §LTA6.2 group line
below it. A per-option reason is a concept that only exists where there are
options to attach it to.
