...
label chip, ✕ and the resize handle are **post-selection** tab stops, so the
keyboard rename path survives without a second stop on every frame: select the
frame, Tab to the label, Enter — which since LGR-D16 / §FC10 opens the frame's
**properties popover** (name + accent) with focus in its name field, rather than
an inline text box. The accent swatches are no longer inner tab stops of the
frame at all: they live in that popover, which is portaled to `document.body`. The four edge hit-strips stay
