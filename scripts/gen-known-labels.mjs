...
// canonical `frameId -> English title` for a template, from examples/<id>.json
function canonicalFrames(tplId) {
  try {
    const doc = JSON.parse(read(`examples/${tplId}.json`))
    return new Map((doc.frames ?? []).map((f) => [f.id, f?.label]))
  } catch {
    return new Map()
  }
}
...
// `frameId -> localized title` for (template, locale) from the frame dict source
function localizedFrames(tplId, locale) {
  const slice = frameDictSlice(locale)
  const block = new RegExp(`'${tplId}'\\s*:\\s*\\{([\\s\\S]*?)\\n\\s*\\}`).exec(slice)
  const out = new Map()
  for (const m of (block?.[1] ?? '').matchAll(
    /^\s*([A-Za-z_$][\w$]*|'[^']+')\s*:\s*'([^']*)'/gm,
...
// ── frame titles (§TLO12) — same shape, own maps ──
const knownFrames = new Map() // frameId -> Set<string>
const frameTargetByLocale = new Map()
const ambiguousFrames = new Set()
const addKnownFrame = (id, label) => {
  if (label == null || label === '') return
  let s = knownFrames.get(id)
  if (!s) knownFrames.set(id, (s = new Set()))
  s.add(label)
}
const setFrameTarget = (locale, id, label) => {
  if (label == null || label === '') return
  let m = frameTargetByLocale.get(locale)
  if (!m) frameTargetByLocale.set(locale, (m = new Map()))
  const prev = m.get(id)
  if (prev !== undefined && prev !== label) ambiguousFrames.add(id)
  else m.set(id, label)
}
for (const tplId of TEMPLATE_IDS) {
  const canon = canonicalFrames(tplId)
  if (canon.size === 0) continue
  for (const [id, enLabel] of canon) {
    addKnownFrame(id, enLabel)
    setFrameTarget(BASE_LOCALE, id, enLabel)
  }
  for (const locale of NON_BASE) {
    if (!DICT_LOCALES.includes(locale)) continue
    const loc = localizedFrames(tplId, locale)
    for (const [id, enLabel] of canon) {
      const label = loc.get(id) ?? enLabel
      addKnownFrame(id, label)
      setFrameTarget(locale, id, label)
    }
  }
}
...
const sortedFrameIds = [...knownFrames.keys()].sort()
const frameEntries = sortedFrameIds
  .map((id) => {
    const labels = [...knownFrames.get(id)].sort().map((s) => JSON.stringify(s))
    return `  ${JSON.stringify(id)}: [${labels.join(', ')}],`
  })
  .join('\n')
const ambiguousFramesArr = [...ambiguousFrames].sort().map((s) => JSON.stringify(s))
 ...
 *  in SOME shipped locale (the ${BASE_LOCALE.toUpperCase()} canonical included). Empty until a
...
export const AMBIGUOUS_FRAME_IDS: readonly string[] = [${
  ambiguousFramesArr.length ? `\n  ${ambiguousFramesArr.join(',\n  ')},\n` : ''
}]
`
