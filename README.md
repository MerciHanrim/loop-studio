...
- ◐ Productization track — making the tool usable by a general planner, not only its author; a separate track from Onboarding. Design-first: each pass is its own doc and PR
  ...
  - ◐ Dense-graph pan usability ([`docs/dense-graph-pan.md`](docs/dense-graph-pan.md), `DGP`) — **Draft PR; a real-phone round found one-finger pan working but two-finger pinch not zooming at all, so the overlay now computes pinch itself instead of handing it to React Flow; re-verification on a real phone is the Ready/merge gate**. On a packed graph there is no empty canvas to grab, so panning is near-impossible (mobile especially; the minimap is only a secondary aid). A transparent pan-capture overlay handles it: a one-finger drag past ~8px pans even when it starts on a node, live on mobile always and on desktop behind a session-only Pan mode toggle. A shorter tap still selects a **node** — or, failing that, the **nearest edge** within ~14px — and opens the Inspector (§DGP-C1). Wheel / trackpad-pinch zoom (mouse) is forwarded untouched; edit gestures are byte-for-byte unchanged when Pan mode is off. Native OS gestures are not suppressed but must never leave the overlay stuck (§DGP-C2). Independent of Focus / Filter / frames / Activity overlay / selection; no GraphDoc / digest / undo change. Sequenced **before** contextual inline help
