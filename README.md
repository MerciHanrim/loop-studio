...
- ◐ Productization track — making the tool usable by a general planner, not only its author; a separate track from Onboarding. Design-first: each pass is its own doc and PR
  ...
  - ◐ Large-graph readability ([`docs/large-graph-readability.md`](docs/large-graph-readability.md)) — design doc settled (rev 3); **Slices 1–2 shipped, Slice 3 next**. Slice 1: a global hit-test rule (a node beats an overlapping edge / badge with Focus off — the fix for mis-clicks) and a selection-driven 1-hop focus view. Slice 2: ephemeral **filters** that hide by edge class / resource type / node kind, every list built from the open graph (not a fixed palette), cleared on graph reload and Reset view. Still to come: the `evaluated` vs `effective` run distinction read straight off `StepReport` (Slice 3); per-step (non-accumulating) cues with an opt-in activity overlay and session-`transient` group frames (Slice 4a); `auto` frames get their own Slice-4b design pass; a `saved` frame is deferred behind a Frozen `loop-revision/N` cosmetic `frames` contract. Per-item persistence table in §LGR3.4. No node re-layout, no engine / GraphDoc / serialize / digest change; render / UI-only
