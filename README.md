...
- ◐ Productization track — making the tool usable by a general planner, not only its author; a separate track from Onboarding. Design-first: each pass is its own doc and PR
  ...
  - ◐ Template label overlay ([`docs/template-label-overlay.md`](docs/template-label-overlay.md)) — a shared **fresh-open** `nodeId → label` overlay so a bundled Template opens in the user's language from **one** English-canonical graph (no per-locale JSON copies). Applied once, on a menu open, current locale only; never re-translates an open document; Import / Share / Workspace / autosave never trigger it; `label` only (not ids / expr / `resourceType` / positions); a CI drift check for missing / stale entries. Template 3 (MMO) adopts it — its KO labels harvested from `mmo-progression.ko.json`, canonical graph untouched. No engine / schema / wire / save-format change. Built before the coffee Template
  - ◐ Coffee-roastery Template ([`docs/example-coffee-roastery.md`](docs/example-coffee-roastery.md)) — the **first external product-direction validation** (triggered by an outside reader reading Loop Studio as a real-business tool), not an LGR slice: a small real-business Graph JSON (≤ 20–25 nodes, one-day step, buy green beans → roast → sell across cafe / retail / online + dessert, five surfaced Parameters, a 4–6 Register Summary) shipped as the **4th Templates entry** (`커피 로스터리 운영`), `examples/coffee-roastery.json` — English canonical labels, Korean via the label overlay, opens editable. Two impl PRs: (1) the overlay, (2) the JSON + registration (`templates.ts`, `templateKeys.ts`, KO dict, 2 catalog keys) — no engine / schema / wire. Design PR first; then a domain-informed external reader picks it from the menu, unaided, to check the model reads. LGR Slice 2 waits on that check
