...
| [`SEMANTICS-S3.md`](SEMANTICS-S3.md) | `loop-state/3` | current: adds `label` `timing: "phase0" \| "afterPull"` gated by `when: "source-fired"` — a new Phase 2.5 (after the pull), a `report.stateEvents` `applied` field for the `afterPull` variant, and a fail-closed validation set (CSU3-5); byte-identical to `loop-state/2` for a document with no `timing` / `when` (or `timing: "phase0"` only, the same default). |
...
| [`SEMANTICS-R6.md`](SEMANTICS-R6.md) | `loop-revision/6` | Revision projection / diff / Apply extended for **CSU / `loop-state/3`** — a `state` edge's `timing` + `when` (already-declared trailing `EDGE_FIELDS.state` keys). Unlike `route` / `waypoints` / `frames`, these are **`engine`**-tagged: they set `engineAffecting` and move both the `loop-revision/*` content digest and the `loop-workspace/1` semantic digest, under the ordinary field-diff / `nConf` rules (no cosmetic carve-out). A per-side v6 predicate (checked first, independent of model / routing / frames) with verify-own-projection-then-lift ordering closes a real gap: a pure engine-only CSU graph (no Parameter/Register/routing/frames) used to be mis-inferred as `loop-revision/1` and crash the `R2-INV-2` lift assertion. Emission is **verbatim, not re-validated** — an unrecognised `timing` / `when` (which the engine fail-closes, `SEMANTICS-S3.md` §S3-5) still moves the digest, deliberately, since it is real behavioural content, not the default. |
| [`SEMANTICS-R8.md`](SEMANTICS-R8.md) | `loop-revision/8` | Revision projection / diff / Apply extended for the **data-import storage foundation** (`docs/data-import.md` §DI16 Phase 1A) — a new graph-level `GraphDoc.dataImports` array (mirroring `frames`' shape) plus four trailing `parameter` fields, `sourceTableId` / `sourceKey` / `sourceColumnId` / `labelAutoComposed` (mirroring CSU's per-node shape), combined into one extension since data-import content is both shapes at once. A new **`provenance`** field tag — distinct from `cosmetic`, `engine`, and `advisory` — since this is real document content that changes nothing the engine computes but changes the meaning of a future Refresh (not yet implemented). Classified by **presence, not validity**: an incoherent partial provenance triple, or a bad reference, is still `loop-revision/8` content, never silently normalised back to plain — each field is read and kept **verbatim and independently**, the CSU precedent, not the `min`/`max` coherent-pair-or-drop one. The `dataImports` array gets `frames`' whole-array-hunk treatment; the four per-node fields get the ordinary per-field `change`-hunk treatment, and are members of `OPTIONAL_PROJECTED_KEYS` from the start. `loop-workspace/1` stays v1 — provenance is not a real input to what a run computes. No CSV/TSV import UI, Refresh, or diff/export workflow yet — Phase 1B, a separate future slice. |
