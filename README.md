...
| [`SEMANTICS-R2.md`](SEMANTICS-R2.md) | `loop-revision/2` *(Draft)* | Revision projection / diff / Apply extended for the `loop-model/1` layer — a syntactic per-graph v1/v2 predicate (inferred, never stored), two new `FIELDS_BY_KIND` rows + a trailing `resourceType`, the `advisory` field tag, a conservative-extension guarantee with a golden vector, "verify the v1 digest, then lift" ordering, and bidirectional v1 ↔ v2 compare/Apply. `loop-workspace/1` stays v1. |
