...
- ◐ Productization track — making the tool usable by a general planner, not only its author; a separate track from Onboarding. Design-first: each pass is its own doc and PR
  ...
  - ◐ Large-graph readability ([`docs/large-graph-readability.md`](docs/large-graph-readability.md)) — design doc settled, implementation pending: a selection-driven focus view, transient de-emphasis / filters, a `pointer-events` discipline so dimmed edges never steal a click, a run distinction between `evaluated` and `effective`, per-step (non-accumulating) cues with an opt-in activity overlay, and group frames — `auto` + session-`transient` ship first (derived / `localStorage`, no wire), a `saved` frame is deferred behind a Frozen `loop-revision/N` cosmetic `frames` contract. No node re-layout, no engine change; Slices 1–4 are render / UI-only
