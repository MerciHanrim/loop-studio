...
- ◐ Productization track — making the tool usable by a general planner, not only its author; a separate track from Onboarding. Design-first: each pass is its own doc and PR
  ...
  - ☐ Dense-graph pan usability ([`docs/dense-graph-pan.md`](docs/dense-graph-pan.md), `DGP`) — **design proposal, not started**. On a packed graph there is no empty canvas to grab, so panning is near-impossible (mobile especially; the minimap is only a secondary aid). Direction: a one-finger drag pans even when it starts on a node / edge (short tap still selects), and on desktop an explicit Pan mode + `Space + drag`. Edit gestures preserved; independent of Focus / Filter / frames / Activity overlay / selection; no GraphDoc / digest / undo change. Sequenced **before** contextual inline help
