...
- ◐ Productization track — making the tool usable by a general planner, not only its author; a separate track from Onboarding. Design-first: each pass is its own doc and PR
  ...
  - ◐ Large-graph readability ([`docs/large-graph-readability.md`](docs/large-graph-readability.md)) — design doc settled (rev 2), implementation pending: a global hit-test rule (node beats an overlapping edge / badge, with Focus off — the fix for mis-clicks), a selection-driven 1-hop focus view, ephemeral de-emphasis / filters, a run distinction between `evaluated` and `effective` derived from the committed step result only, per-step (non-accumulating) cues with an opt-in activity overlay, and group frames — session-`transient` ships in Slice 4a, `auto` frames get their own Slice-4b design pass, a `saved` frame is deferred behind a Frozen `loop-revision/N` cosmetic `frames` contract. Per-item persistence table in §LGR3.4. No node re-layout, no engine change; Slices 1–4a are render / UI-only
