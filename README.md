...
- ◐ Productization track — making the tool usable by a general planner, not only its author; a separate track from Onboarding. Design-first: each pass is its own doc and PR
  ...
  - ◐ Coffee-roastery Import prototype ([`docs/example-coffee-roastery.md`](docs/example-coffee-roastery.md)) — a product-direction validation artefact, not an LGR slice: a small real-business Graph JSON (≤ 20–25 nodes, one-day step, buy green beans → roast → sell across three channels + dessert, five surfaced Parameters, a 4–6 Register Summary) built as an **Import-only** `examples/coffee-roastery.ko.json` — Korean display names, stable English node ids — **not** menu-registered. The comprehension check is run with a coffee-business reviewer, unaided, in Korean (measures the model, not English). Design doc first; the JSON lands in a separate impl PR; it becomes the 4th Templates entry only if the check passes. No app / engine / schema change. LGR Slice 2 waits on this
