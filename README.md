...
- ◐ Productization track — making the tool usable by a general planner, not only its author; a separate track from Onboarding. Design-first: each pass is its own doc and PR
  ...
  - ◐ Large-graph readability ([`docs/large-graph-readability.md`](docs/large-graph-readability.md)) — design doc settled (rev 3), implementation pending: a global hit-test rule (node beats an overlapping edge / badge, with Focus off — the fix for mis-clicks), a selection-driven 1-hop focus view, ephemeral de-emphasis / filters (resource-type list built from the free-form `resourceType` strings in the open graph; the eight real node kinds), a run distinction read straight off `StepReport` (`evaluated` = a node in `activated` but not `fired`, node-only in v1; edges are `effective` or get no cue), per-step (non-accumulating) cues with an opt-in activity overlay, and group frames — session-`transient` ships in Slice 4a, `auto` frames get their own Slice-4b design pass, a `saved` frame is deferred behind a Frozen `loop-revision/N` cosmetic `frames` contract. Per-item persistence table in §LGR3.4. No node re-layout, no engine change; Slices 1–4a are render / UI-only
  ...
  - order settled ([`docs/product-direction.md`](docs/product-direction.md) §PD8): large-graph readability first (read/select problem already reproducible in the shipped Early MMO example, smaller scope, lower serialization risk, and its focus/filter substrate is a dependency of the module system's assembly screen), then the small module / template system
  - ✅ Template label overlay ([`docs/template-label-overlay.md`](docs/template-label-overlay.md)) — a shared **fresh-open** `nodeId → label` overlay so a bundled Template opens in the user's language from **one** English-canonical graph (no per-locale JSON copies). Applied once, on a menu open, current locale only; never re-translates an open document; Import / Share / Workspace / autosave never trigger it; `label` only (not ids / expr / `resourceType` / positions); a CI drift check for missing / stale entries. Template 3 (MMO) adopts it — its KO labels harvested from `mmo-progression.ko.json`, canonical graph untouched. No engine / schema / wire / save-format change. Shipped as two PRs (`b938aed`)
  - ◐ Coffee-roastery Template ([`docs/example-coffee-roastery.md`](docs/example-coffee-roastery.md)) — the **first external product-direction validation** (triggered by an outside reader reading Loop Studio as a real-business tool), not an LGR slice. An **operational-flow simulation example** — an assumption-based model of green / roasted stock, sales, stockouts, waste and profit — explicitly **not** a real-time inventory / POS / ERP tool (an external reader flagged that scope risk). Small Graph JSON (≤ 20–25 nodes, one-day step, buy green beans → roast → sell across cafe / retail / online + dessert), five **operational levers**, a 4–6 Register Summary, shipped as the **4th Templates entry** (`커피 로스터리 운영 흐름 시뮬레이션`); English canonical labels, Korean via the label overlay, opens editable. **rev 6 (in review)**: impl PR (2) is paused — the frozen `loop-model/1` + `loop-expr/1` engine does not let a `parameter` node drive the run, so §CR16 compares two directions (a minimal *general* `parameter → simulation input` feature, or a smaller engine-unchanged model) for the reviewers to pick before any code. LGR Slice 2 waits on the external check
