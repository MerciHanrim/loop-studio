// docs/template-label-overlay.md §TLO2 — the Korean node-`label` overlay for
// bundled Templates. `label` only — never `resourceType`, which stays the
// canonical English advisory value in every locale. `mmo-progression`'s Korean
// labels were harvested once from a hand-made `examples/mmo-progression.ko.json`
// derived copy; that harvest is permanent and the file was removed (an
// independent per-locale graph would only be a bad precedent — §TLO-D1).
// `check:template-labels` keeps every block in sync with its canonical graph
// (`examples/<id>.json`).

import type { TemplateLabelDict, TemplateLabelMap } from './index'

export const ko: TemplateLabelDict = {
  // The two production-line samples — structure is the frozen SEMANTICS §14
  // Variant A / Variant B; only the labels carry the production framing.
  'equilibrium': {
    'tpl-src': '원료 공급',
    'tpl-vault': '원료 재고',
    'tpl-gate': '생산 배분',
    'tpl-conv': '가공',
    'tpl-prod': '완제품 재고',
    'tpl-spill': '폐기',
    'tpl-consume': '출하',
  },
  'deadlock': {
    'tpl-src': '원료 공급',
    'tpl-vault': '원료 재고',
    'tpl-gate': '생산 배분',
    'tpl-conv': '가공',
    'tpl-prod': '완제품 재고',
    'tpl-spill': '폐기',
  },
  'mmo-progression': {
    level: '레벨',
    xp: '경험치',
    xp_earned: '획득 경험치',
    reward: '보상',
    fail_pool: '실패',
    death_pool: '사망 대기열',
    reward_router: '보상 분배기',
    hunt_payout: '사냥 보상 지급',
    quest_payout: '퀘스트 보상 지급',
    hunt_xp: '사냥 경험치',
    quest_xp: '퀘스트 경험치',
    fail_conv: '실패 비용',
    death_conv: '사망 비용',
    combat_wins: '전투 승리',
    combat_fails: '전투 실패',
    deaths: '사망 (회)',
    gold: '골드',
    gold_earned: '획득 골드',
    vendor_revenue: '상인 판매 수익',
    repair_spend: '수리 비용',
    resupply_spend: '재보급 비용',
    training_spend: '훈련 비용',
    water: '물 (단위)',
    food: '식량 (단위)',
    water_bought: '구매한 물 (단위)',
    food_bought: '구매한 식량 (단위)',
    water_consumed: '소비한 물 (단위)',
    food_consumed: '소비한 식량 (단위)',
    water_upkeep: '물 유지',
    food_upkeep: '식량 유지',
    resupply: '재보급',
    gear_score: '장비 점수',
    gear_wear: '장비 마모도',
    wear_cleared: '마모 해소',
    repair_wear: '수리(마모)',
    repair_gold: '수리(비용)',
    drop: '드롭',
    loot_feed: '분류 대기 전리품',
    loot_dispatch: '전리품 분배',
    loot_category: '전리품 분류',
    bucket_equip: '장비 드롭',
    bucket_vendor: '판매용 드롭',
    bucket_consumable: '소모품 드롭',
    bucket_rare: '희귀 드롭',
    items_looted: '획득 아이템',
    items_equipped: '장착 아이템',
    items_sold: '판매 아이템',
    items_consumed: '소비 아이템',
    equip_conv: '장착',
    vendor_conv: '상인에게 판매',
    consumable_conv: '소모품 사용',
    rare_conv: '희귀 아이템 판매',
    elapsed: '경과 단계',
    clock: '시계',
    completion: '완료',
    completion_src: '완료 신호',
    end15: '15레벨 도달',
    z1_enc_src: '초원 지대 조우',
    z1_enc: '초원 지대 · 1–5레벨',
    z1_combat: '초원 지대 전투',
    z1_win: '초원 지대 승리',
    z1_winamp: '초원 지대 승전 처리',
    z1_lootroll: '초원 지대 전리품 판정',
    z1_loot: '초원 지대 전리품',
    z1_xp_meter: '초원 지대 경험치 계량기',
    z1_xp2lvl: '초원 지대 레벨업',
    z1_training: '초원 지대 훈련',
    z2_enc_src: '구릉 지대 조우',
    z2_enc: '구릉 지대 · 5–10레벨',
    z2_combat: '구릉 지대 전투',
    z2_win: '구릉 지대 승리',
    z2_winamp: '구릉 지대 승전 처리',
    z2_lootroll: '구릉 지대 전리품 판정',
    z2_loot: '구릉 지대 전리품',
    z2_xp_meter: '구릉 지대 경험치 계량기',
    z2_xp2lvl: '구릉 지대 레벨업',
    z2_training: '구릉 지대 훈련',
    z3_enc_src: '고지대 조우',
    z3_enc: '고지대 · 10–15레벨',
    z3_combat: '고지대 전투',
    z3_win: '고지대 승리',
    z3_winamp: '고지대 승전 처리',
    z3_lootroll: '고지대 전리품 판정',
    z3_loot: '고지대 전리품',
    z3_xp_meter: '고지대 경험치 계량기',
    z3_xp2lvl: '고지대 레벨업',
    z3_training: '고지대 훈련',
    void: '드롭 없음',
    char_creation: '캐릭터 생성',
    active_char: '활성 캐릭터',
    r_income: '총 수입',
    r_expense: '총 지출',
    r_netgold: '순 골드 검산',
    r_huntshare: '사냥 경험치 비율',
    r_efflevel: '경험치 진행 지수',
    r_items_acct: '아이템 정산',
    r_burned: '소비한 소모품',
  },
  // "Coffee roastery operations flow" (docs/example-coffee-roastery.md §CR12.1).
  // Physical-flow terms use natural industry Korean; the two signed-proxy
  // read-outs use "… 여유" — never `놓친 판매` / `품절 손실` / `폐기`. `label`
  // only — ids / expressions / positions stay English (§TLO-D4).
  'coffee-roastery': {
    cafe_retail_demand_kg: '카페·리테일 원두 수요량 (kg/일)',
    daily_roast_kg: '하루 로스팅량 (kg)',
    online_orders: '온라인 원두 주문량 (kg/일)',
    green_wholesale_kg: '생두 납품 주문량 (kg)',
    dessert_prep: '하루 디저트 준비량',
    green_delivery: '생두 입고',
    green_stock: '생두 재고',
    green_wholesale: '생두 납품',
    roasting: '로스팅 · 수율 82%',
    roasted_stock: '로스팅 원두 재고',
    roast_loss: '로스팅 중량 감소',
    online_sales: '온라인 포장 원두 판매',
    cafe_retail: '카페·리테일 원두 사용',
    roasted_bleed: '직원·커핑·샘플용',
    dessert_prep_src: '디저트 준비',
    dessert_stock: '디저트 재고',
    dessert_sales: '디저트 판매',
    dessert_wrapup: '마감 잔량',
    projected_revenue: '하루 예상 매출',
    planned_cost: '하루 계획 비용',
    projected_operating_margin: '하루 예상 운영차익',
    roasted_supply_margin: '로스팅 원두 수급 여유',
    dessert_prep_margin: '디저트 준비 여유',
  },
  // docs/gacha-banner-zones.md (GZ) — node ids come straight from
  // src/engine/gachaBannerZonesGraph.ts's builder (also `examples/
  // gacha-banner-zones.json`); `check:template-labels` fails if this drifts.
  'gacha-banner-zones': {
    pulls_per_zone: '존별 뽑기 횟수',
    hit_rate_free: '적중률 — 일반(무료)',
    hit_rate_standard: '적중률 — 프리미엄 상시',
    hit_rate_pickup: '적중률 — 프리미엄 픽업',
    pickup_share_pickup: '픽업 비율 — 프리미엄 픽업',
    fund_free: '티켓 지급',
    ticket_free: '티켓',
    pulls_made_free: '뽑기 횟수',
    zone1_free_w_ssr: 'SSR 가중치',
    zone1_free_w_sr: 'SR 가중치',
    zone1_free_w_r: 'R 가중치',
    sr_count_free: 'SR 횟수',
    r_count_free: 'R 횟수',
    ssr_count_free: 'SSR 횟수',
    roll_gate_free: '뽑기 판정',
    ssr_hit_free: 'SSR 결과',
    sr_hit_free: 'SR 결과',
    r_hit_free: 'R 결과',
    fund_standard: '티켓 지급',
    ticket_standard: '티켓',
    pulls_made_standard: '뽑기 횟수',
    zone2_standard_w_ssr: 'SSR 가중치',
    zone2_standard_w_sr: 'SR 가중치',
    zone2_standard_w_r: 'R 가중치',
    sr_count_standard: 'SR 횟수',
    r_count_standard: 'R 횟수',
    ssr_count_standard: 'SSR 횟수',
    roll_gate_standard: '뽑기 판정',
    ssr_hit_standard: 'SSR 결과',
    sr_hit_standard: 'SR 결과',
    r_hit_standard: 'R 결과',
    pity_standard: '천장 카운트',
    zone2_standard_hard_pity: '하드 천장',
    forced_ssr_standard: '강제 SSR',
    ceiling_hits_standard: '천장 발동 횟수',
    fund_pickup: '티켓 지급',
    ticket_pickup: '티켓',
    pulls_made_pickup: '뽑기 횟수',
    zone3_pickup_w_ssr: 'SSR 가중치',
    zone3_pickup_w_sr: 'SR 가중치',
    zone3_pickup_w_r: 'R 가중치',
    sr_count_pickup: 'SR 횟수',
    r_count_pickup: 'R 횟수',
    ssr_count_pickup: 'SSR 횟수',
    pity_pickup: '천장 카운트',
    zone3_pickup_hard_pity: '하드 천장',
    ceiling_hits_pickup: '천장 발동 횟수',
    missed_pickup_pickup: '픽업 보장 상태',
    zone3_pickup_w_pickup: '픽업 가중치',
    zone3_pickup_w_standard: '일반 가중치',
    roll_normal_open_pickup: '뽑기 — 일반, 보장 없음',
    roll_normal_owed_pickup: '뽑기 — 일반, 보장 있음',
    roll_forced_open_pickup: '뽑기 — 강제, 보장 없음',
    roll_forced_owed_pickup: '뽑기 — 강제, 보장 있음',
    ssr_split_open_pickup: 'SSR 분배 — 보장 없음',
    pickup_hit_pickup: '픽업 적중',
    standard_hit_pickup: '일반 적중',
    sr_hit_pickup: 'SR 결과',
    r_hit_pickup: 'R 결과',
    pickup_count_pickup: '픽업 횟수',
    standard_count_pickup: '일반 횟수',
  },
}

// docs/template-label-overlay.md §TLO12 — the Korean group-frame TITLE overlay,
// `templateId -> (frameId -> title)`. Flat sibling of `ko` above so the
// source-text checks parse it independently. `check:template-labels` fails if a
// shipped frame lacks an entry here (or vice-versa).
export const koFrames: TemplateLabelMap = {
  'coffee-roastery': {
    zone_supply: '공급·재고',
    zone_roasting: '로스팅·판매',
    zone_forecast: '예측 지표',
  },
  'gacha-banner-zones': {
    zone_free: '일반(무료)',
    zone_standard: '프리미엄 상시',
    zone_pickup: '프리미엄 픽업',
    zone_comparison: '비교',
  },
}
