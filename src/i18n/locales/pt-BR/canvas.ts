// docs/localization.md §L3.3 — Canvas surface slice of the `pt-BR` catalog.
// Brazilian Portuguese: `arquivo` never `ficheiro`, `tela` never `ecrã`,
// `salvar` never `guardar`, `usuário` never `utilizador`, `caractere` never
// `carácter`, sentence case, `“…”` for user-facing prose.
//
// GLOSSARY, one word per node kind (the 8 x 3 contract in `ptBrCopy.test.ts`):
...
const canvas = {
  ...
  'palette.converter.description': 'Consome recursos de entrada e produz recursos de saída na proporção que você definir. Não guarda nada.',
  ...
  'palette.parameter.description': 'Um número fixo que você define. Não tem portas, e uma expressão pode referenciá-lo pelo id.',
  ...
  'canvas.filter.groupEdgeClass': 'Tipo de conexão',
  ...
  'canvas.regionSelect.on': 'Selecionar uma região — selecionando; arraste na tela vazia, Esc para cancelar',
  ...
  'canvas.frame.drawing': 'Quadro de grupo — desenhando; arraste na tela vazia, Esc para cancelar',
  'canvas.frame.defaultName': 'Grupo {n}',
  'canvas.frame.delete': 'Excluir este quadro',
  ...
  'canvas.frame.suggestNote': 'Grupos estruturais sugeridos — podem não corresponder à forma como você dividiria o trabalho.',
  ...
  'canvas.edgeLabel.breakdown.title': 'as transferências deste passo por esta conexão',
  'canvas.edgeLabel.refMissing': 'Erro de referência de Parâmetro',
  'node.unreadable.title': '{kind} ilegível',
  'node.unreadable.sub': 'não é possível ler os dados — corrija no arquivo',
  ...
  'canvas.frame.a11y.desc': 'Pressione Enter ou Espaço para selecionar este quadro.',
  ...
  'rf.dir.up': 'cima',
  'rf.dir.down': 'baixo',
  'rf.controls.label': 'Controles da tela',
  ...
  'rf.handle.label': 'Ponto de conexão',
  'rf.node.a11y': 'Pressione Enter ou Espaço para selecionar este nó. Pressione Delete para removê-lo, Escape para cancelar.',
  'rf.node.a11yKeyboard': 'Pressione Enter ou Espaço para selecionar este nó e depois as teclas de seta para movê-lo. Pressione Delete para removê-lo, Escape para cancelar.',
  'rf.edge.a11y': 'Pressione Enter ou Espaço para selecionar esta conexão. Pressione Delete para removê-la, Escape para cancelar.',
} as const
