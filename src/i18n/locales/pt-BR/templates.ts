// docs/localization.md §L3.3 — Templates & modules slice of the `pt-BR`
// catalog. Brazilian Portuguese (§L2.14): `arquivo` never `ficheiro`, `tela`
// never `ecrã`, `salvar` never `guardar`, `usuário` never `utilizador`,
// `caractere` never `carácter`, sentence case, `“…”` for user-facing prose.
//
...
//
// **`Template` is deliberately NOT `Modelo`.** `Modelo` is the standard
// Brazilian word for a document template, but `modelo` is already this
// product's word for the simulation MODEL — 10 keys and 14 occurrences say
// "the model", "a v2 model", "model versions", "run the model". Using one word
// for both would make `modules.promote.*` read as though loading a Template
// changed the model version. `Template` is the word Brazilian game- and
// dev-tool users already use for a starting-point document, so the anglicism
// is the clearer of the two (native review pending).
//
...
const templates = {
  ...
  'templates.deadlock.blurb': 'A mesma linha sem a etapa de expedição: o estoque enche até o limite e tudo para.',
  ...
  'templates.coffeeRoastery.blurb': 'Como torra, vendas e estoque puxam uns contra os outros ao longo de um dia de operação.',
  ...
  'modules.promote.body': 'Inserir este bloco converte o documento em um modelo v2 e altera o resumo de semântica do modelo. Um único desfazer reverte de uma vez a mudança de modelo e a inserção.',
  'modules.promote.confirm': 'Converter e inserir',
  'modules.frames.title': 'Os quadros salvos não são incluídos',
  'modules.frames.insertBody': 'Este arquivo tem quadros de grupo salvos. Inseri-lo como módulo não traz os quadros para o seu grafo; todo o resto é inserido normalmente.',
  'modules.frames.extractBody': 'Seu grafo tem quadros de grupo salvos. Eles não são gravados no arquivo do módulo: somente os nós selecionados e suas conexões internas são.',
