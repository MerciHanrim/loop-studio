...
const ko = {
  ...
  // docs/large-graph-readability-auto-frames.md §AF — 프레임 제안 (Slice 4b)
  ...
  'inspector.edge.flowParam.notParam': '“{id}” 은(는) 파라미터가 아닙니다 — 이 연결의 흐름은 0입니다',
  'inspector.edge.flowParam.malformed': '올바른 파라미터 참조가 아닙니다 — 이 연결의 흐름은 0입니다',
  'inspector.edge.flowParam.hint':
    '파라미터 참조는 id로 유지됩니다. 파라미터 이름을 바꿔도 되지만, 삭제하면 참조가 남으며 자동으로 고쳐지지 않습니다.',
  ...
  'templates.equilibrium.blurb':
    '원료가 들어오고, 생산이 가공과 폐기로 나뉘며, 완제품이 출하됩니다. 재생해 보세요 — 원료 재고와 완제품 재고가 몇 단계 만에 안정되고 Timeline이 평평하게 유지됩니다.',
  'templates.deadlock.name': '용량 교착',
  'templates.deadlock.blurb':
    '같은 라인에서 출하 단계가 없어 완제품이 빠져나갈 곳이 없습니다. 재생해 보세요 — 완제품 재고가 용량까지 차고, 원료 재고가 천장까지 역류하며, 공급이 0으로 조여지고 라인 전체가 멈춥니다.',
  ...
  'templates.coffeeRoastery.blurb':
    '로스팅·판매·재고의 관계를 단순화해 살펴보는 운영 흐름 시뮬레이션: 생두가 입고되고, 일부는 납품으로 나가며, 나머지는 로스팅해 카페·온라인·리테일로 판매됩니다. 하루 운영 값 다섯 개를 바꾸며 재고 궤적과 예상 지표가 어떻게 움직이는지 살펴보세요. 단순화한 시뮬레이션 예제이며 ERP나 실시간 모니터링 시스템이 아닙니다.',
