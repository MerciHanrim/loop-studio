// FROZEN ORACLE — do not "improve", do not re-sync with the production module.
//
// A verbatim copy of `src/components/edges/orthogonalRoute.ts` as it stood on
// `main` at e763eeb, before the §ER3 router's internals were cost-reduced. It
// exists so `orthogonalRoute.differential.test.ts` can assert that the shipped
// router is BYTE-IDENTICAL to the pre-optimisation behaviour on inputs far
// beyond what a stored fixture can hold (seeded random layouts, the stress
// graph, every boundary case).
//
// Test-only: never imported by the app. If a future change is MEANT to alter
// routes, this file is not what gets edited — `ROUTER_VERSION` is bumped and
// the fixtures are regenerated in the same reviewed change.
//
// Generated by: git show e763eeb:src/components/edges/orthogonalRoute.ts
//   + `export ` stripped from the module-level declarations
//   + `computeOrthogonalRoute` renamed to `computeOrthogonalRouteReference`
...
type RouteClass = 'orthogonal' | 'self-loop' | 'same-side' | 'fallback-lz' | 'degenerate'
...
// ── the entry point ───────────────────────────────────────────────────────
export function computeOrthogonalRouteReference(inp: RouteInput): RouteResult {
